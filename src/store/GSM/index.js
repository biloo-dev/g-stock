...
var { encrypt,key,token, decrypt, EncryptionFile, DecryptionFile} = require('./Activation');
 ...
 * Possible options:
 *  ports
 *  forever (if set to true, will keep trying to connect to modem even after it is disconnected. 
 *  This can facilitate running this module as a daemon)
 *  debug
 *  auto_hangup
 *  ussdTimeout
 *  commandsTimeout
...
function Modem(opts) {
  ...
  // Auto hang up calls
  this.autoHangup = opts.auto_hangup || true;

  this.ussdTimeout = opts.ussdTimeout || 35000;
  this.commandsTimeout = opts.commandsTimeout || 35000;
  ...
  for (i; i < this.ports.length; ++i) {
    if (cb) {
        ...
        return new Promise((res,rej)=>{
            this.connectPort(this.ports[i], (err,data)=>{  
                if (err) {
                    rej(err)
                } else {
                    res(data)
...
Modem.prototype.connectPort = function (port, cb) { 
  ...
  var buf = Buffer.alloc(256*1024), bufCursor = 0;
...
Modem.prototype.sendSMS = function (message, cb) {
  ...
  if (message.receiver === undefined || message.text === undefined) {
    cb(('Either receiver or text is not specified'));
    return;
  }
...
/**
 * Returns modem's get Operators #######################################################################
 */
Modem.prototype.getSimNumber = function (cb) {
  "use strict"; 
  return new Promise(async (res,rej)=>{
    let result = await this.sendCommand("AT+CNUM", (err, data) => {
 ...
 * Returns modem's get Operators #######################################################################
 */
Modem.prototype.getOperators = function (cb) {
  "use strict"; 
  return new Promise(async (res,rej)=>{
    let result = await this.customATCommand("AT+COPS?")  
    let resu = result.match(/"(.*?)"/) ? result.match(/"(.*?)"/)[1] : null;
    if ("Djezzy,60302".indexOf(resu) != -1) {
      this.operator = 'Djezzy'  
      res("Djezzy") 
    } else if ("ATM Mobilis,60301".indexOf(resu) != -1) {
      this.operator = 'Mobilis'  
      res("Mobilis") 
    } else if ("60303,Ooredoo,Ooredoo@".indexOf(resu) != -1) {
...
/**
 * Requests custom USSD get Balance
 */
 
Modem.prototype.ResUssd = function (cmd) {
  return new Promise((resolve, reject) => {
    console.log('cmd :>> ', cmd);
    this.getUSSD('AT+CUSD=1,"' + cmd + '"', (err, res) => {
        console.log('res :>> ', res);
      console.log('err :>> ', err);
      if (res) {
        resolve(res)
...
Modem.prototype.Flexy = function (cmd) {
  "use strict";  
  return new Promise(async (resolve,reject)=>{
    console.log('cmd :>> ', cmd);
    await this.customATCommand('AT+CUSD=1 \r\n')
    await this.getUSSD('AT+CUSD=1,"' + cmd + '",15',(err,res)=>{  
      console.log('res :>> ', res);
      console.log('err :>> ', err);
          if (res) { 
              resolve(res) 
...
Modem.prototype.getBalance = function (cmd) {
  "use strict";  
  return new Promise(async (resolve,reject)=>{
    console.log('cmd :>> ', cmd);
    await this.getUSSD('AT+CUSD=1,"*' + cmd + '#",15',(err,res)=>{  
...
/**
 * Return Active Sim Card
 */
Modem.prototype.ActiveSim = async function (key) {
  "use strict";  
  return new Promise(async(res,rej)=>{
    try { 
      let license = await decrypt(key) 
      let IMSI = await this.getIMSI()
      let IMEI = await this.getIMEI()
      let lisLocal =parseInt(IMSI) + parseInt(IMEI) ; 
       
        if (license == lisLocal) { 
          await settings.set('settings.isTry',false);
          await settings.set('settings.'+this.operator,{license:key});
          this.isActive = true
          res(true) 
        }else {
          this.isActive = false
...
/**
 * Returns Virify Activation Sim
 */
Modem.prototype.isActivate = async function (cb) {
  "use strict"; 
  let stng = await settings.get('settings'); 
  return new Promise(async(res,rej)=>{
    try {
      let key = stng[this.operator] ? stng[this.operator].license : '' 
      let license = await decrypt(key) 
      let IMSI = await this.getIMSI()
      let IMEI = await this.getIMEI()
      let lisLocal =parseInt(IMSI) + parseInt(IMEI) ; 
      console.log('lisLocal,license :>> ',this.operator, lisLocal,license);
      if (license == lisLocal) {   
         res(true) 
        this.isActive = true
 ...
 */
Modem.prototype.getIMEI = async function (cb) {
  "use strict"; 
  return new Promise((res,rej)=>{
    this.sendCommand('AT+CGSN', function (data) { 
        ...
        if (null !== match && match.length === 2) {
          ...
          rej(('GET IMEI NOT SUPPORTED: ' + data));
        } 
    });
  })
};
...
Modem.prototype.getOperator = function (text, cb) {
  ...
  this.sendCommand('AT+COPS=3,' + (text ? '0' : '2') + ';+COPS?', function (operator) {
      ...
      if (typeof cb === 'function') {
        cb(undefined, operator);
      if (null !== match && 4 < match.length) {
        cb(undefined, match[3]);
      } else {
        cb(('GET OPERATOR NOT SUPPORTED'));
      }
    }
  }.bind(this));
};
  ...
  this.sendCommand('AT+CLIP=?', function (data) {
    ...
    if (typeof cb === 'function') {
      if (match) {
        ...
        cb(('GET CALLER ID NOT SUPPORTED'));
      }
    }
  }.bind(this));
};
     ...
     var match = data.match(/\+CSQ: (\d+),(\d*)/); 
      if (null !== match && 2 < match.length) {
        ...
        if (scale === 99) {
          ...
          var db = -113 + scale * 2,
           condition, icon = 'mdi-signal-cellular-outline';
          if (db < -95) {
            condition = 'bad one-bar';
            icon = 'mdi-signal-cellular-1'
          } else if (db < -85) {
            condition = 'bad two-bars';
            icon = 'mdi-signal-cellular-2'
          } else if (db < -75) {
            condition = 'ok three-bars';
            icon = 'mdi-signal-cellular-3'
          } else {
            condition = 'good four-bars';
            icon = 'mdi-signal-cellular-3'
          }
          cb(undefined, {
            db: db,
